...
- **P3 进行中（媒体/RKNN DSO 已落地，板卡回归未完成）**：后端 DSO 加载器完成（`lib/backend_dso.c`：可信目录
  ...
  （`backends/backend_mpp.c`）实现 `mpp.decode`（H.264/HEVC/AV1 +
  AUTO/转码首包 Annex-B 探测）与 `mpp.encode`（H.264/HEVC，DMA-BUF
  零拷贝/HOST 拷贝、CBR/FIXQP），由发布管线 `deps-mpp` 阶段交叉构建并
  随包分发 MPP 运行库；`rkvc_frame_spec` 新增 `ver_stride`。RGA 后端 DSO
  （`backends/backend_rga.c`）实现 `rga.scale`（NV12/NV21/YUV420P/RGB24、
  HOST/DMA-BUF 输入 → 线性 HOST 输出，bicubic 缩放；显式几何或 UPSCALE
  默认 2×），librga 以子模块提交为锚、由 `deps-rga` 适配器装配 SDK 前缀
  并随包分发运行库。x86 契约测试 `test_media_pipeline`（9 用例）与交叉包
  verify/QEMU 冒烟常绿。
  RKNN 后端 DSO（`backends/backend_rknn.c`）已消费 bind_model 交付的
  已校验载荷，直接 `rknn_init` 并严格验证 Phase-RLFN 12→108 契约，完成
  NV12 3× 推理与残差融合；fake RKNN x86 端到端契约测试已覆盖实际调用链。
  **MPP 媒体路径板卡回归已完成**（RK3576 + RV1126B，2026-09-02）：
  统一 CLI 的 `inspect device`、H.264/HEVC decode（240p/720p/1080p）、
  encode、transcode、bench 全部通过，全程直接使用包内 MPP 运行库；
  交叉构建缺陷（isoc23 符号泄漏、解码死锁）已根治并进入 verify 门禁。
  **RGA/RKNN 硬件功能与板卡回归已验证**（RK3576 板：RGA upscale 346fps、
  MPP decode/encode/transcode/bench 455fps、RKNN NPU 3x 60.8fps；
  RV1126B 板：RGA + RKNN NPU 3x 17.9fps 自动选模型）；RK3588（权威板卡）因
  sshd 异常未测，待恢复后补测。
- **P4 完成（跨板模型自动选择已验证）**：`.rkmodel` v1 线格式
  ...
  只淘汰；`rknn_target` 按 SoC 族名前缀匹配 `caps.soc`，兼容
  `rk3576` ↔ `rk3576-evb1-v10`、`rv1126b` ↔ `rv1126b-evb4-v10`）；
  写入/签名侧 `tools/rkvc_build/rkmodel.py`
  ...
  已进入 job 创建校验；自动模型选择经 `rkvc_node_ops.bind_model` 驱动
  transform 工厂：图构建期按请求选定模型、SHA-256 校验后把容器摘要与
  RKNN 载荷交付首选候选，无兼容模型或契约拒绝时淘汰该候选并回退，
  载荷篡改以 INTEGRITY 拒绝且不回退（`test_model_binding` 三用例覆盖）。
  CLI `rkvc inspect models` 已接真实注册表，`rkvc upscale` 已接
  file.source(NV12) → transform → file.sink 链路（RGA 路径可用，
  `--model` 可覆盖自动选择）。`rknn.upscale` 已消费 bind_model（量化
  模型输入属性 INT8/zp=-128 视为 uint8 字节流的量化重解释，接受该约定）；
  统一 CLI 已补齐内建 `bench`（同 Request/Job 路径预热、多轮采样、
  FPS/实时倍速、JSON）与 `license`（许可证标识/全文/第三方声明定位）。
  退出条件已验证：同一 0.4.0 通用包（P07）在 RK3576 与 RV1126B 两类
  RKNN 目标设备上不传 `--model` 即自动选择匹配模型完成 NPU 3x upscale
  （640×368 → 1920×1104，字节精确），bench 分别 60.8fps / 17.9fps。
...
- **P6 已完成旧架构切除**：0.3 Session/Pipeline/Router、旧节点实现、七个
  独立 CLI、绑定旧 Session 的示例与专用测试、手工打包脚本和 `RKVC_1.0` 符号节点均已
  删除。0.4 核心直接产出 `librkvc.so.0`，不再存在 `rkvc_core` 迁移目标或
  `RKVC_BUILD_NEW_ENGINE` 开关；文件、流式端口、ROI 与自适应码率示例已按
Context/Request/Job/Frame 重写，旧测试行为也迁入 0.4 契约测试。RGA 与
Phase-RLFN RKNN SR 已由 0.4 后端 DSO 接替。
- **P7 完成（MLVC 神经编解码迁入 0.4 后端架构，2026-09-02）**：
  新增 `backends/backend_mlvc.c` DSO（工厂 `mlvc.encode`/`mlvc.decode`，
  priority 1100）与支撑库 `backends/mlvc/`（`rans`/`mlvc_pixel`/`pmf`/
  `qppatch`/`container`，容器无 `node_mlvc.c` 特殊 run loop）。模型经
  `.rkmodel` 多载荷交付：`rknn` + `pmf-gaussian` + `pmf-bitest`
  （可选 `qppatch`），SHA-256 由图内核校验后经 `bind_model` 传入；
  `RKVC_CODEC_MLVC` 进入公共 codec 枚举与 CLI（`.mlvc` 扩展名自动推断）。
  x86 fake RKNN 契约测试 `test_backend_mlvc`（2 用例：容器头/记录守恒、
  编解码往返）常绿，全套 ctest 14/14。真实模型已从 microsoft/mlvc
  公开 checkpoint 导出（640×368、qp=21 折叠进图、tail D2S 拆出图外）：
  rk3576 与 rv1126b 双平台 `.rkmodel` 各自打包。
  **板卡回归通过**（RK3576：encode 20 帧 2.8s、decode 2.2s，100 帧×3 轮
  码流 md5 逐字节一致，Y-PSNR 32.7dB；RV1126B：encode 20 帧 5.1s、
  decode 4.6s，Y-PSNR 32.71dB；跨板互操作：RK3576 编码流在 RV1126B
  完整解码）。RK3588（权威板卡）sshd 异常，待恢复后补测。

剩余硬件门禁项：RGA/RKNN 板卡回归（含 RK3588 权威板卡恢复后补测
MPP 基线），以及 RKNN SR/MLVC 的质量与长稳门禁。
MPP 媒体路径已在 RK3576/RV1126B 完成板卡回归（2026-09-02）；
RGA 与 Phase-RLFN RKNN SR 的后端 DSO 已可进入发布管线，但硬件功能
未在板卡验证。执行顺序为 RK3588（权威板卡）→ RK3576 → RV1126B；
每块板均运行统一 CLI 的 `inspect device` 和 H.264/HEVC/AV1 能力范围
内的媒体基线。板卡访问：仅 Windows 侧网络可达（IP/SSH 见环境配置，
不入库）。
...
| 七个 `cli/rkvc_*.c`                                       | 根 `rkvc.c`（按需拆分共享 parser/output）                 | 形成一个二进制和稳定退出码               |
