...
- **P3 进行中（媒体/RKNN DSO 已落地，板卡回归未完成）**：后端 DSO 加载器完成（`lib/backend_dso.c`：可信目录
  ...
  **MPP/RGA/RKNN 硬件功能与板卡回归尚未验证**；P3 不能仅凭
  加载器、fake backend 或 x86_64/QEMU 结果判定闭环。板卡在线上，但
  板卡回归（RK3588 → RK3576 → RV1126B）尚未启动。
- **P4 进行中（容器、运行时绑定与统一 CLI 完成，跨板验证未完成）**：`.rkmodel` v1 线格式
  ...
  `--model` 可覆盖自动选择）。`rknn.upscale` 已消费 bind_model；统一 CLI
  已补齐内建 `bench`（同 Request/Job 路径预热、多轮采样、FPS/实时倍速、
  JSON）与 `license`（许可证标识/全文/第三方声明定位）。P4 仍需同一通用包
  在至少两种 RKNN 目标设备完成自动模型选择，才能按退出条件标记完成。
...
- **P6 已完成旧架构切除**：0.3 Session/Pipeline/Router、旧节点实现、七个
  ...
  Context/Request/Job/Frame 重写，旧测试行为也迁入 0.4 契约测试。未迁移的
  MLVC 神经编解码功能暂不可用，不回退旧路径；RGA 与 Phase-RLFN RKNN SR
  已由 0.4 后端 DSO 接替。

剩余硬件门禁项：真实板卡回归，以及 RKNN SR/MLVC 的质量与长稳门禁。
MPP/RGA/RKNN 已 DSO 化并可进入发布管线，但硬件功能未在板卡验证。
执行顺序为 RK3588（权威板卡）→ RK3576 → RV1126B；每块板均运行
统一 CLI 的 `inspect device` 和 H.264/HEVC/AV1 能力范围内的媒体基线。
板卡在线：当前仓库无板卡接入方式（IP/SSH 凭据）入库，待补充访问信息后
即可启动回归。
