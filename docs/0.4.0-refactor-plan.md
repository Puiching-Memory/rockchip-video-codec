...
- **P3 进行中（文件管线与 MPP DSO 落地，板卡回归未完成）**：后端 DSO 加载器完成（`lib/backend_dso.c`：可信目录
  ...
  verify/QEMU 冒烟常绿。
  **MPP/RGA 硬件功能、RKNN 后端与板卡回归尚未验证**；P3 不能仅凭
...
- **P4 进行中（容器与信任链完成，运行时绑定未完成）**：`.rkmodel` v1 线格式
  ...
  `--model` 可覆盖自动选择）。RKNN 推理 transform 尚未落地，消费
  bind_model 的 NPU 候选仍缺位，P4 仍不能按端到端口径标记完成。
...
- **P6 已完成旧架构切除**：0.3 Session/Pipeline/Router、旧节点实现、七个
  ...
  RGA/RKNN/SR/MLVC 功能暂不可用，不回退旧路径。

剩余硬件门禁项：RKNN 后端 DSO 化、`bench` CLI 平价、真实板卡回归。
MPP/RGA 已 DSO 化并进入发布管线，但硬件功能未在板卡验证。
