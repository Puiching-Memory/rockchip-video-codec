...
- **P2 完成**：`context/request/job/frame/diagnostic` 公共 ABI 冻结于
  `include/rkvc/`；图内核（两步构建、有界队列背压、逆序回滚、确定性候选
  顺序）在 `lib/graph.c`/`executor.c`/`job.c`；`librkvc.map` 符号白名单
  分 RKVC_1.0（0.3 旧符号）与 RKVC_0.4 两节点。`test_graph_executor` /
  `test_job` 独立编译常绿。
- **P3 进行中**：后端 DSO 加载器完成（`lib/backend_dso.c`：可信目录
  dlopen、`rkvc_backend_query()` ABI 握手、失败隔离、淘汰诊断、
  `RTLD_NOW|RTLD_LOCAL`）；内建后端注册点 `lib/builtin_backends.c`。
  MPP/RGA/RKNN 节点逐节点迁移需真实板卡，当前在 x86_64 容器内只能到
  编译验证层面——这是 P3 未闭环的唯一原因。
- **P4 完成（容器与信任链）**：`.rkmodel` v1 线格式
  ...
  Python 签名 → C 验证互操作已闭环（trust=development）。
...
- **P6 进行中**：`scripts/package-portable.sh` 已标记弃用（stderr 横幅），
  但 0.3 旧媒体栈（`lib/session*`、旧 CLI）仍是媒体功能唯一载体，
  在 P3 硬件迁移完成并经板卡验证前不得删除；统一 CLI 的媒体子命令
  如实报告"本构建未包含媒体后端"（退出码 1）。

剩余硬件门禁项：MPP/RGA/RKNN 后端 DSO 化、媒体 CLI 平价、真实板卡回归
（`rkvc-bench`/`inspect device` 在 RK3576/RK3588/RV1126B）。
