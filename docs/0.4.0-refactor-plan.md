...
- **P2 完成（核心契约）**：`context/request/job/frame/diagnostic` 公共 ABI
  ...
  MPP/RGA/RKNN 资源。新引擎独立发布符号冻结在 `librkvc-0.4.map` 的
  `RKVC_0.4` 节点；严格告警与 ASan/UBSan 回归常绿。
- **P3 进行中（框架完成，生产后端未完成）**：后端 DSO 加载器完成（`lib/backend_dso.c`：可信目录
  dlopen、`rkvc_backend_query()` ABI 握手、失败隔离、淘汰诊断、
  `RTLD_NOW|RTLD_LOCAL`）；工厂描述符在注册期校验，单候选损坏或打开失败
  不再拖垮其他路径。`lib/builtin_backends.c` 仍为空注册点，MPP/RGA/RKNN
  与软件媒体节点尚未实现为 0.4 DSO；P3 必须完成实际节点适配和板卡验证，
  不能仅凭加载器、fake backend 或 x86_64 编译判定闭环。
...
`rkvc-bench`、`inspect device` 和 H.264/HEVC/AV1 能力范围内的媒体基线。
