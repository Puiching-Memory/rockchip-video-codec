...
# SR phase 像素契约 + ONNX/bundle 导出契约
ctest --preset tests -R 'test_sr_(phase|export)' --output-on-failure
...
# NPU / Phase-RLFN 超分门禁（需 models/rkvc-sr/ 完整 bundle）
...
- 实机通过 `./scripts/test-rga.sh` 与 `./scripts/test-npu-sr.sh`（需 `models/rkvc-sr/` 完整 bundle）
- 新缺陷附带回归测试
