...
| `docs/`（全套文档）、`examples/`（C ABI 样板）、`CHANGELOG.md` | codec 插件开发用的 `plugin.hpp`        |
| `licenses/`、可选 `models/`、`test.sh`、`MANIFEST.sha256` | doxide 生成的 `docs/api/`（现生成、不入库）|
...
包里那份 `examples/`（三个 C ABI 样板）同样要源码树才能构建
（`-DRKVC_CORE_DIR=<core 目录>`），随包是作契约样板与参考实现用；
`docs/` 则是可以直接阅读的全套文档，入口 `docs/index.md`。
