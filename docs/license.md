...
这套 `RKVC_ENABLE_LICENSE` 产品授权与可移植包的 `RKVC_ENABLE_MODEL_CRYPT` 模型访问控制是两个独立开关。后者默认由 `package-portable.sh` 为 NPU 模型启用，可用 `--no-encrypt-models` 关闭；标准包随附只读的 `rkvc_model_id` 采集机器码。模型加密使用客户端内嵌的对称密钥，只用于提高直接复制门槛，不具备 Ed25519 授权方案“客户端无法取得签发私钥”的安全性质。
