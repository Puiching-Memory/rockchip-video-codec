...
/*
 * backends/mlvc/rans.h — rANS 熵编解码器（纯 C 实现）。
 *
 * 完整移植自 Microsoft msrtc_rans（mlvc/packages/msrtc_rans，C++17 模板），
 * 覆盖 RansByte/Rans64 双变体、预计算编码符号表、CDF 二分解码、
 * 流式 + 一次性编解码、bypass 离群值与 EOF 校验。
 *
 * 算法来源：ryg_rans / msrtc_rans（MIT License, Copyright (c) Microsoft Corporation）。
 */

#ifndef RKVC_BACKEND_MLVC_RANS_H
#define RKVC_BACKEND_MLVC_RANS_H
...
#endif /* RKVC_BACKEND_MLVC_RANS_H */
