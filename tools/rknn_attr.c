...
int main(int argc, char **argv) {
    ...
    for (uint32_t i = 0; i < io.n_input; ++i) {
        ...
        printf(
            "in[%u] %s: n_dims=%d dims=[%d,%d,%d,%d] fmt=%d type=%d qnt=%d fl=%d zp=%d scale=%.6f n_elems=%u size=%u\n",
    ...
    for (uint32_t i = 0; i < io.n_output; ++i) {
        ...
        printf(
            "out[%u] %s: n_dims=%d dims=[%d,%d,%d,%d] fmt=%d type=%d qnt=%d fl=%d zp=%d scale=%.6f n_elems=%u size=%u\n",
            i, a.name, a.n_dims, a.dims[0], a.dims[1], a.dims[2], a.dims[3],
            (int)a.fmt, (int)a.type, (int)a.qnt_type, a.fl, a.zp, a.scale,
            a.n_elems, a.size);
    }
