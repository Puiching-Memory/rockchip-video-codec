...
SVG。四种编码器使用固定配色，原生编码与重建消融分图展示，避免十几条曲线
...
- 码率取完整输出码流字节数（含容器／头部）×8÷源序列时长，不能用目标 bitrate
  或网络估计熵替代。QP 数值不跨编码器等价；至少四档，按**实际码率**排序。
...
- 当前 MPP 默认为 30 fps、GOP 60；SVT 默认为 30 fps、intra_period_length 60
  （其日志 GOP 61）、preset 8。CLI 不暴露统一 GOP/FPS，结果属于当前板端实现
  对比，不能声称统一 CTC、公平软件编码器排名或硬件 AV1 编码。
