...
uint32_t rotr(uint32_t x, unsigned n) noexcept {
    return (x >> n) | (x << (32 - n));
}
...
std::array<uint8_t, 32> sha256(const uint8_t* data, size_t len) noexcept {
    ...
    if (rem + 1 + 8 <= 64) {
        for (size_t i = rem + 1; i < 56; ++i)
            buf[i] = 0;
        for (int i = 0; i < 8; ++i)
            buf[56 + i] = static_cast<uint8_t>(bit_len >> (56 - i * 8));
        block(buf);
    } else {
        for (size_t i = rem + 1; i < 128; ++i)
            buf[i] = 0;
        block(buf);
        for (int i = 0; i < 120; ++i)
            buf[i] = 0;
        for (int i = 0; i < 8; ++i)
            buf[120 + i] = static_cast<uint8_t>(bit_len >> (56 - i * 8));
        block(buf + 64);
    }
